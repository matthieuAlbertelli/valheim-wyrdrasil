...
public sealed class RegistryHudRenderer
{
        ...
        int residentCount)
    {
        ...
        var panelRect = new Rect(20f, 180f, 760f, 338f);
        ...
        GUI.Label(new Rect(35f, 266f, 700f, 20f), $"Action : {FormatAction(state.SelectedAction)}", _textStyle!);
        ...
        GUI.Label(new Rect(35f, 434f, 700f, 20f), $"Résident sélectionné pour force assign : {FormatPendingResidentForceAssign(state)}", _textStyle!);
        GUI.Label(new Rect(35f, 458f, 700f, 20f), "Astuce : avec 'Force assign', vise d'abord un PNJ enregistré, puis vise un slot ou un siège.", _hintStyle!);
        GUI.Label(
            new Rect(35f, 482f, 700f, 20f),
            $"{toggleKey} : mode | {nextCategoryKey} : catégorie | {nextActionKey} : action | Clic gauche : créer/éditer | Clic droit : supprimer",
            _hintStyle!);
    }
